...
check("v1 data survives migration",
      recipes.length === 3 && recipes[0].name === "Alpha",
      recipes.map(r => r.name).join(","));
check("every recipe got an id",
      recipes.every(r => typeof r.id === "string" && r.id.length > 8));
check("ids are unique", new Set(recipes.map(r => r.id)).size === 3);
check("every ingredient got an id",
      recipes.every(r => r.ingredients.every(i => typeof i.id === "string" && i.id.length > 8)));
check("updatedAt populated", recipes.every(r => Number.isFinite(r.updatedAt)));
check("deletedAt defaults to null", recipes.every(r => r.deletedAt === null));
check("null amount preserved through migration",
      recipes[0].ingredients[1].amount === null,
      JSON.stringify(recipes[0].ingredients[1]));
...
check("migrated recipes default to private",
      recipes.every(r => r.visibility === "private"),
      recipes.map(r => r.visibility).join(","));
check("owner defaults to null", recipes.every(r => r.owner === null));
check("publishedAs defaults to null", recipes.every(r => r.publishedAs === null));

// Idempotence is what makes it safe to run migrate() on every load forever.
const idsBefore = recipes.map(r => r.id).join(",");
const twice = migrate(migrate({ schemaVersion: 2, recipes }));
...
check("ids stable across a reload",
      loadRecipes().map(r => r.id).join(",") === idsBefore);

// --- the bug that index-based identity caused ---
const alphaId = recipes[0].id;
const gammaId = recipes[2].id;
showRecipe(gammaId);
...
// --- soft delete ---
check("tombstone kept in memory", recipes.length === 3 && liveRecipes().length === 2);
check("tombstone has deletedAt", recipes.find(r => r.id === alphaId).deletedAt > 0);
check("tombstone persisted to disk", stored().recipes.length === 3,
      "stored=" + stored().recipes.length);
check("findRecipe refuses a tombstone", findRecipe(alphaId) === null);
...
check("unknown id hides display",
      recipeDisplay.classList.contains("d-none") && currentRecipeId === null);
...
showRecipe(gammaId);
const ingIdBefore = findRecipe(gammaId).ingredients[0].id;
editRecipeBtn.click();
servingsInput.value = "16";
saveRecipeBtn.click();
const after = findRecipe(gammaId);
...
check("new recipe is displayed after save", recipeName.textContent === "Delta",
      recipeName.textContent);
check("new recipe selected in dropdown", recipeSelect.value === currentRecipeId);
check("new recipe got an id",
      typeof currentRecipeId === "string" && currentRecipeId.length > 8);
check("new recipe has a tombstone field", findRecipe(currentRecipeId).deletedAt === null);

// Opening "Add" must not disown the recipe already on screen.
showRecipe(gammaId);
addRecipeBtn.click();
modal("recipeModal").hide();
check("add modal leaves current recipe intact", currentRecipeId === gammaId,
